...
//
// File: google-sheet-API.js
//
// Su dung Sheet API clone:
// API_T08.26
// API_T09.26
//
// Doc configAPI.json
...
async function taiConfig(forceReload = false) {
    ...
    if (!response.ok) {
        ...
        throw new Error(
            "Khong tai duoc configAPI.json. HTTP: " +
            response.status
        );
...
async function layThongTinSheet(maThang) {
    ...
    const spreadsheetId =
        laySpreadsheetId(thongTin.link);

    if (!spreadsheetId) {

        throw new Error(
            "Khong lay duoc Spreadsheet ID cua: " +
            maThang
        );
...
async function traCuuMaLenh(maLenh) {
    ...
    try {
        ...
        // Lay thong tin Sheet API
        // tu configAPI.json

        const thongTin =
            await layThongTinSheet(maThang);


        // Query tim chinh xac Ma Lenh
        ...
        // Goi truc tiep Google Visualization API
        ...
        // Chi doi dau cham thanh dau phay
        // trong Kich Don.
        //
        // Khong anh huong Don Hang
        // hoac Ma Lenh.
        ...
        // de index.html khong can thay doi
